...
public class AuthConfiguration {
	 ...
	 @Autowired
	    ...
	    /*metodo per la password criptata*/
	    ...
	    /*definisce il filtro HTTP che gestisce le policies di autenticazione e autorizzazione attraverso una lunga serie di invocazioni concatenate all'oggetto httpSecurity: i metodi restituiscono lo stesso oggetto http aggiornato..*/
	    @Bean
	    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
	        httpSecurity
	                .csrf().disable()
	                .authorizeRequests()
	                .requestMatchers(HttpMethod.GET, "/", "/index", "/index.html", "/register", "/css/**", "/images/**", "/favicon.ico", "/classifica", "/libri", "/libro/**", "/autore/**" ).permitAll()
	                ...
	                .and()
	                .exceptionHandling()
	                .accessDeniedPage("/erroreAutorizzazioneadmin.html") //Aggiunto questo
	                .and()
	                .formLogin()
	                .loginPage("/login")
	                .permitAll()
